...
// updateKey resolves the verification key: ORBIT_UPDATE_KEY over the
// operator's settings.json updateKey — never the rig embedded key, the
// releases are signed with orbit's own minisign key. Unpinned refuses.
func updateKey(getenv func(string) string) (string, error) {
	if v := strings.TrimSpace(getenv("ORBIT_UPDATE_KEY")); v != "" {
		return v, nil
	}
	...
	raw, ok := keys["updateKey"]
	if !ok {
		return "", nil
	}
	var v string
	if err := json.Unmarshal(raw, &v); err != nil {
		return "", fmt.Errorf("update: settings.json updateKey: %w", err)
	}
...
func update(ctx context.Context, cfg updateCfg) error {
	...
	case cmp == 0:
		fmt.Fprintf(cfg.out, "orbit: already at %s (latest)\n", cfg.version)
		return nil
	}
	...
	if strings.TrimSpace(cfg.key) == "" {
		return fmt.Errorf("update: no verification key: set ORBIT_UPDATE_KEY or settings.json updateKey to the minisign public key that signs the releases (minisign -G); an unpinned update is refused")
	}
	...
	if err != nil {
		return fmt.Errorf("cannot write %s: %v — grant write access to %s and retry (e.g. sudo orbit -update)", dir, err, dir)
	}
	tmpPath := tmp.Name()
	...
	fmt.Fprintf(cfg.out, "orbit: %s -> %s (%s)\n", cfg.version, latest, cfg.bin)
...
func assetName(goos, goarch string) (string, error) {
	...
	return "orbit_" + goos + "_" + goarch, nil
}
