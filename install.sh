#!/bin/sh
# orbit installer: fetch the release binary for this platform, verify it,
# land it in ${ORBIT_BIN:-$HOME/.local/bin} - never sudo. Every failure
# names what it was doing.
set -eu

repo="mrsirg97-rgb/orbit"
...
install -m 0755 "$tmp/$asset" "$bindir/orbit" || { echo "install: install into $bindir failed" >&2; exit 1; }

case ":$PATH:" in
  *":$bindir:"*) ;;
  *) echo "install: $bindir is not on your PATH; add it:"; echo "  export PATH=\"\$PATH:$bindir\"" ;;
esac
"$bindir/orbit" -version
